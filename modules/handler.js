...
function registerIpcHandlers(getMainWindow, sendTransferUpdate, sendSpooferResult, sendStatus) {
  ...
  async function runSpoofer(data) {
    ...
    if (data.downloadOnly && !data.downloadFolder?.trim())
      return fail('Please select a download folder for Download-Only mode.');
    ...
    if (data.groupId && !/^\d+$/.test(String(data.groupId).trim()))
      return fail(`Invalid Group ID "${data.groupId}" — must be a number.`);
    ...
    if (!data.animationId?.trim()) return fail('No asset IDs provided.');

    const entries = parseAssetInput(data.animationId);
    if (!entries.length) return fail(`No valid ${assetType} entries.`);
    ...
    if (!cookieHeader) return fail('Invalid ROBLOSECURITY cookie format.');
    ...
    const downloadOne = async entry => {
      ...
      if (loc?.locations?.[0]?.location) {
        ...
        const batchErr =
          loc?.errors?.[0]?.Message || loc?.errors?.[0]?.message || 'No location in batch response';
        for (const url of [
    ...
    const listFails = (label, items) => {
      if (!items.length) return '';
      const lines = items
        .slice(0, 5)
        .map(f => `- ${f.name} (${f.id}) — [${f.category}] ${f.reason}`);
      return `${label}:\n${lines.join('\n')}${items.length > 5 ? `\n(+${items.length - 5} more…)` : ''}\n`;
    };
